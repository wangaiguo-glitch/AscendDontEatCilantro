...
def get_parser_train(parents=None):
    ...
    parser.add_argument("--per_batch_size", type=int, default=16, help="per batch size for each device")
    ...
    parser.add_argument("--opencv_threads_num", type=int, default=1, help="set the number of threads for opencv")
...
def train(args):
    ...
    for stage in range(len(stage_epochs)):
        ...
        _dataloader = create_loader(
            ...
            shuffle=True,
            drop_remainder=True,
            num_parallel_workers=1, # args.data.num_parallel_workers
            python_multiprocessing=False, # True
        )
